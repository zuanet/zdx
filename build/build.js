...
class Build {
    constructor() {
        ...
        if(this.args.folder)
            ...
            this.FOLDER = path.join(os.homedir(),'.zua-desktop','build');
        ...
        this.BIN = path.join(os.homedir(),'.zua-desktop','bin',this.PLATFORM_ARCH);
    ...
    help() {
        const text = 
`Zua-Desktop builder ${pkg.version}
Usage: build <flags>
Where <flags> are:
...
`;
    ...
    async main() {
        try {
            ...
            this.log(`Zua-Desktop builder ${pkg.version}`);
    ...
    async build() {
        try {
            ...
            this.TOOLS = path.join(this.FOLDER,'tools');
            // this has migrated to ~/.zua-desktop/bin 
            ...
            const dest = path.join(this.GOSRC,'github.com/zuadanet/');
            ...
            if(this.args['with-extras']) {
                ...
                for(const repo of repos.extras) {
                    this.log(`git clone git@github.com:zuadanet/${repo}`);
                    await this.clone(`git@github.com:zuadanet/${repo}`, dest);
                }
            }
	...
	download(url, file) {
        ...
        return new Promise((resolve, reject) => {
            ...
            progress(
                request({ url, headers: { 'User-Agent': 'Zua-Desktop' } }), 
