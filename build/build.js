...
class Build {
    constructor() {
        ...
        if(this.args.folder)
            ...
            this.FOLDER = path.join(os.homedir(),'.zdx','build');
        ...
        this.BIN = path.join(os.homedir(),'.zdx','bin',this.PLATFORM_ARCH);
...
`;
    ...
    async main() {
        try {
            ...
            this.log(`ZDX builder ${pkg.version}`);
    ...
    async build() {
        try {
            ...
            const dest = path.join(this.GOSRC,'github.com/zuanet/');
            ...
            if(this.args['with-extras']) {
                ...
                for(const repo of repos.extras) {
                    this.log(`git clone git@github.com:zuanet/${repo}`);
                    await this.clone(`git@github.com:zuanet/${repo}`, dest);
                }
            }
            ...
            let targets = [
                ...
                'zua/zuasyncd',
                'zua/examples/wallet',
                ...fs.readdirSync(path.join(dest,'zuad/cmd')).map(f => `zuad/cmd/${f}`)
            ];
            ...
            fse.copy(path.join(dest,'zuanetwork','zua','database','migrations'),path.join(this.BIN,'database','migrations'));
	...
	download(url, file) {
        ...
        return new Promise((resolve, reject) => {
            ...
            progress(
                request({ url, headers: { 'User-Agent': 'ZDX' } }), 
...
Before building Zua, you need to do some setup:
From the zdx folder:
