...
class ZuaProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['zuad','mqtt','mysql','pgsql','zuasyncd','zuanetworkd', 'gpuminer', 'zuaminer','txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['zuad','mqtt','mysql','pgsql','zuasyncd','zuanetworkd','simulator','txgen'];
		//const typeMap = { };
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, zdx } = pkg;
			return {
				folder, name, description, main, engines, ...zdx
			}
		});
	}
