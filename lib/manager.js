...
class ZuaProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['zuad','mqtt','mysql','pgsql','gpuminer','zuaminer','txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['zuad','mqtt','mysql','pgsql' ,'txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.zua-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, zua-desktop } = pkg;
			return {
				folder, name, description, main, engines, ...zua-desktop
			}
		});
	}
...
module.exports = ZuaProcessManager;
