...
const {Client:ZuaGRPC} = require('@zua/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Zuad extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			if (!("uacomment" in args)) {
				const uacomment = `zua-desktop_${pkg['version']}`
				this.writeToSink(`ZUAD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`ZUAD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`ZUAD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`ZUAD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`ZUAD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'zuad.conf');
			...
			await this.VSARGS([this.getDeployableBinary('zuad'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('zuad')
					];
			...
			resolve();
			// mutes all zuad processes by default
			this.mute = true;
		})
	}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('ZUAD monitor:'.bold,ex);
	            return reject(ex);
	        }
