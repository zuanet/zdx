...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@zua/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@zua/ux"] = "N/A";
window.PWA_MODULES["@zua/grpc-web"] = "N/A";
window.PWA_MODULES["@zua/wallet"] = "N/A";
window.PWA_MODULES["@zua/grpc"] = "N/A";
window.PWA_MODULES["@zua/core-lib"] = "N/A";

class ZDXApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"ZDX"}
		let list = [
                        ['Zua','MIT','Copyright (c) 2024 Zua Developers'],
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		return html`
		<flow-caption-bar
			logo="/resources/images/zua-logo-light-bg.png"><font style="color:#b89e14;">ZUA</font></flow-caption-bar>
		<tab-content for="home">
			...
			<div id='zua-resources'><font style="color:#b85e14;">

				<flow-expandable>
					<div slot="title" is="i18n-div" caption>ZUA RESOURCES</div>
					<ul style="font-size: 12px;">                                                                                                                                                   <li><flow-shell-link                                                                                                                                                            href="http://zuanet.com/"><flow-i18n>Website</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
							href="https://github.com/zuanet/zuad">GitHub</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link                                                                                                                  				     href="https://github.com/zuanet/zdx/releases"><flow-i18n>Zdx releases</flow-i18n></flow-shel-link></li>
                                                <li><flow-shell-link
							href="https://discord.gg/PnjCJp9Jmx"><flow-i18n>Discord</flow-i18n></flow-shell-link></li>
                                                <li><flow-shell-link
                                                        href="https://t.me/zuanet"><flow-i18n>Telegram</flow-i18n></flow-shell-link></li>
                                                <li><flow-shell-link
                                                        href="https://miningpoolstats.stream/zuabaniov"><flow-i18n>Miningpoolstats</flow-shell-link></li>
			...
			<flow-form-control icon="fal:copyright">
				<flow-i18n>ZDX &amp; Zua Copyright (c) Zua and Kaspa Developers<br/>
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all ZDX modules. 
					In default configuration this includes Zuad blockchain data and Zua API database.
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor ZDX services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, ZDX runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, ZDX compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			<flow-form-control icon="fal:cube" class="advanced-tool"
				...
				<p slot="info" is="i18n-p">
					The Enable Mining option starts / stops all configured Zuaminer instances.
				</p>
			</flow-form-control>
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream ZDX metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize ZDX environment.
					ZDX configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made ZDX configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart zuad node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the zuad node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
			<div style="height:192px;"></div>
		</tab-content>
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<zua-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></zua-wallet>
		</tab-content>
		<tab-content for="console" data-active-display="flex" class="vertical-flex term">
			<flow-terminal id="zdx-console" class="x-terminal" background="#000" foreground="#FFF"></flow-terminal>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'zdx'});
		this.rpc = rpc;
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='zuad' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='zuad')
				return
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('zua-wallet');
	...
	async initWallet() {
		...
		let settings = await this.get_default_local_zuad_settings();
	...
	async get_default_local_zuad_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let zuad = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
			const [type, ident] = k.split(':');
			return { type, ident, args};
		}).filter(o=>o.type=='zuad').shift();

		if(!zuad)
			return null;//{network:"zua", port:46005};//{network:"zuatest", port:46005};
	...
	async initConsole() {
		let terminal = this.qS('#zdx-console');
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/zua-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("zua-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/zua-logo-${this.theme||"light"}-bg.png`;
	...
	async initTemplates() {
		...
		if(!this.tpl_template) {
			...
			let miner = Object.keys(config.modules).filter(v=>/^(zua|gpu)miner/.test(v));
			if(!miner.length)
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "zdx.$HOSTNAME";
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to ZUA-ZDX ${pkg.version}!
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('zuad:', daemons);
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/zdx-icon.png"
					resizable
					frame
