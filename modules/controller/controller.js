...
class Zua-DesktopApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"Zua-Desktop"}
		
		return html`
		<flow-caption-bar
			logo="/resources/images/Zua-logo-light-bg.png">Zua-Desktop</flow-caption-bar>
		<tab-content for="home">
			...
			<flow-form-control icon="fal:copyright">
				<flow-i18n>Zua-Desktop &amp; Zua Copyright (c) 2024 Zua Developers<br/>
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all Zua-Desktop modules. 
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor Zua-Desktop services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, Zua-Desktop runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, Zua-Desktop compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			...
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream Zua-Desktop metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize Zua-Desktop environment.
					Zua-Desktop configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made Zua-Desktop configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
		...
		<tab-content for="console" data-active-display="flex" class="vertical-flex term">
			<flow-terminal id="zua-desktop-console" class="x-terminal" background="#000" foreground="#FFF"></flow-terminal>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'zua-desktop'});
		this.rpc = rpc;
	...
	async get_default_local_zuad_settings() {
		...
		if(!zuad)
			return null;//{network:"zua", port:16110};//{network:"zuatest", port:16110};
	...
	async initConsole() {
		let terminal = this.qS('#zua-desktop-console');
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "zua-desktop.$HOSTNAME";
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to Zua-Desktop ${pkg.version}!
...
${changelogContent}`;
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.zua-desktop) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/zua-desktop-icon.png"
					resizable
					frame
	...
	async checkForUpdates(force = false) {
		...
		const url = 'https://zua-desktop.app/version.json';
		...
		if(!data) {
			console.log("missing version data in https://zua-desktop.app/version.json");
			return false;
		}
		...
		if(is_gt) {
			...
			let {btn} = await FlowDialog.show({
				title:i18n.t("Zua-Desktop Update"),
			...
			if(btn == 'ok') {
				require('nw.gui').Shell.openExternal('https://zua-desktop.app');

				let confirm = await FlowDialog.show({
					title:i18n.t("Zua-Desktop Update"),
					body:html`Please download the latest version from  <flow-link href="https://zua-desktop.app" target="_blank">https://zua-desktop.app</flow-link>
					<br/>&nbsp;<br/>
					Zua-Desktop will now shutdown`,
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
					},{
...
Zua-DesktopApp.define("zua-desktop-app")
