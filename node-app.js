...
class NodeApp extends App{
	...
	async main() {
		...
		if(this.args.purge || this.args.reset || argv.includes('reset') || argv.includes('purge')) {
			try {
				...
				await fse.remove(path.join(os.homedir(),'.zua-desktop'));
		...
		const ZuaProcessManager = require(path.join(this.appFolder, "lib/manager.js"));
		this.manager = new ZuaProcessManager(null, this.dataFolder, this.appFolder);
	...
	*/
	dataDirInitError(){
		console.error(`Please start app with --init=/path/to/data/dir or --init for default (~/.zua-desktop/data)`.red);
		this.exit();
	}
