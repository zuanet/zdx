...
class App extends FlowApp{
	...
	async initConfig(){
		...
		if(reset) {
			try {
				...
				alert('Error resetting ZDX data folder: '+ex);
			}
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.zdx/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "zuad-zd0", "zua-mainnet", "datadir2");
	...
	*/
	...
	getDefaultConfig(){
		...
		Object.entries(config.modules).forEach(([k,v]) => {
			...
			if(['zuad','zuanetworkd','zuasyncd','zuaminer'].includes(type)) {
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("zuad:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("zuad:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('zuad:') && !('upnpEnabled' in this.config.modules[k])) {
