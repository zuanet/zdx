...
class App extends FlowApp{
	...
	async initConfig(){
		...
		if(reset) {
			try {
				...
				alert('Error resetting Zua-Desktop data folder: '+ex);
			}
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.zua-desktop/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "zuad-kd0", "zua-mainnet", "datadir2");
	...
	*/
	...
	getDefaultConfig(){
		...
		Object.entries(config.modules).forEach(([k,v]) => {
			...
			if(['zuad' ,'zuaminer'].includes(type)) {
