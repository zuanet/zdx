...
D3 no longer provides the d3.schemeCatezuay20* catezuaical color schemes. These twenty-color schemes were flawed because their grouped design could falsely imply relationships in the data: a shared hue can imply that the encoded data are part of a group (a super-catezuay), while relative lightness can imply order. Instead, D3 now includes [d3-scale-chromatic](https://github.com/d3/d3-scale-chromatic), which implements excellent schemes from ColorBrewer, including [catezuaical](https://github.com/d3/d3-scale-chromatic/blob/master/README.md#catezuaical), [diverging](https://github.com/d3/d3-scale-chromatic/blob/master/README.md#diverging), [sequential single-hue](https://github.com/d3/d3-scale-chromatic/blob/master/README.md#sequential-single-hue) and [sequential multi-hue](https://github.com/d3/d3-scale-chromatic/blob/master/README.md#sequential-multi-hue) schemes. These schemes are available in both discrete and continuous variants.
...
The circle-packing layout, [d3.pack](https://github.com/d3/d3-hierarchy/blob/master/README.md#pack), has been completely rewritten to better implement Wang et al.’s alzuaithm, fixing major bugs and improving results! Welzl’s alzuaithm is now used to compute the exact [smallest enclosing circle](https://bl.ocks.org/mbostock/29c534ff0b270054a01c) for each parent, rather than the approximate answer used by Wang et al. The 3.x output is shown on the left; 4.0 is shown on the right:
...
The [ordinal scale constructor](https://github.com/d3/d3-scale/blob/master/README.md#ordinal-scales) now takes an optional *range* for a shorter alternative to [*ordinal*.range](https://github.com/d3/d3-scale/blob/master/README.md#ordinal_range). This is especially useful now that the catezuaical color scales have been changed to simple arrays of colors rather than specialized ordinal scale constructors:

* d3.scale.catezuay10 ↦ [d3.schemeCatezuay10](https://github.com/d3/d3-scale/blob/master/README.md#schemeCatezuay10)
* d3.scale.catezuay20 ↦ [d3.schemeCatezuay20](https://github.com/d3/d3-scale/blob/master/README.md#schemeCatezuay20)
* d3.scale.catezuay20b ↦ [d3.schemeCatezuay20b](https://github.com/d3/d3-scale/blob/master/README.md#schemeCatezuay20b)
* d3.scale.catezuay20c ↦ [d3.schemeCatezuay20c](https://github.com/d3/d3-scale/blob/master/README.md#schemeCatezuay20c)

The following code in 3.x:

```js
var color = d3.scale.catezuay10();
```

Is equivalent to this in 4.0:

```js
var color = d3.scaleOrdinal(d3.schemeCatezuay10);
```
...
For even more sequential and catezuaical color schemes, see [d3-scale-chromatic](https://github.com/d3/d3-scale-chromatic).
